...
async function updateNav() {
  ...
  if (!navCenter && !navRight && !navLinks) return;

  var activePath = window.location.pathname.split('/').pop() || 'index.html';

  var centerLinkClass = 'px-5 py-2 text-base font-semibold rounded-lg transition-colors duration-200 no-underline';
  function cLink(href, label) {
    var isActive = activePath === href;
    return '<a href="' + href + '" class="' + centerLinkClass + (isActive ? ' text-primary bg-primary-light' : ' text-slate-500 hover:text-ink hover:bg-primary-light') + '">' + label + '</a>';
  }

  var centerHtml = cLink('index.html', '首页') + cLink('announcements.html', '公告') + cLink('messages.html', '留言空间');
  ...
  if (user) {
    ...
    rightHtml =
      '<a href="create.html" class="inline-flex items-center gap-1 bg-emerald-600 text-white rounded-full px-4 py-1.5 text-xs font-semibold shadow-sm shadow-green-900/5 hover:bg-primary-hover transition-all duration-200 no-underline">发帖</a>' +
      (admin ? '<a href="admin.html" class="inline-flex items-center bg-amber-500 text-white rounded-full px-4 py-1.5 text-xs font-semibold hover:bg-amber-600 transition-all duration-200 no-underline ml-1">管理</a>' : '') +
      '<a href="profile.html" class="flex items-center gap-1.5 px-3 py-1.5 text-sm text-slate-500 hover:text-ink rounded-lg hover:bg-primary-light transition-colors duration-200 no-underline ml-1" title="个人主页">' +
      ...
      '</a>' +
      '<a href="#" onclick="logout(); return false;" class="px-3 py-2 text-sm text-slate-400 hover:text-red-500 transition-colors duration-200 no-underline ml-1">退出</a>';
  } else {
    rightHtml = '<a href="#" onclick="showLoginModal(); return false;" class="inline-flex items-center bg-emerald-600 text-white rounded-full px-4 py-1.5 text-xs font-semibold shadow-sm shadow-green-900/5 hover:bg-primary-hover transition-all duration-200 no-underline">登录/注册</a>';
  }

  if (navCenter) navCenter.innerHTML = centerHtml;
  if (navRight) navRight.innerHTML = rightHtml;
  if (navLinks && !navCenter) navLinks.innerHTML = centerHtml + rightHtml;
}
